...
class MedicoRepositoryTest {
	...
	void escolherMedicoAleatorioLivreNaDataCenario1() {
		// given ou arrange -  "dado que" - cadastro de informações
		var proximaSegundaAs10 = LocalDate.now()
				.with(TemporalAdjusters.next(DayOfWeek.MONDAY))
				.atTime(10, 0);
		var medico = cadastrarMedico("Médico", "medico@voll.med", "123456", Especialidade.CARDIOLOGIA);
		var paciente = cadastrarPaciente("Paciente", "paciente@emaiil.com", "12345678999");
		cadastrarConsulta(medico, paciente, proximaSegundaAs10);
		
		// when ou act - "quando" dispararmos alguma ação
		var medicoLivre = medicoRepository.escolherMedicoAleatorioLivreNaData(Especialidade.CARDIOLOGIA, proximaSegundaAs10);
		
		// then ou assert - "este" é o resultado esperado
		assertThat(medicoLivre).isNull();
	}
	
	@Test
	@DisplayName("Deveria devolver medico quando ele estiver disponívelna data ")
